...
<div class="flex items-center justify-center min-h-screen bg-gray-100">
    ...
    <h3 class="text-2xl font-bold text-center">Upload Post</h3>
        <form method="post" action="{{route('post.put',$post -> id)}}" enctype="multipart/form-data">
		@method ('PUT')	
        @csrf
            <div class="mt-4">
                <div>
                ...
                <div class="mt-4">
                    <label class="block ">Image<label>
                        <img id="picha" src="{{asset('storage/'.$post -> image)}}" alt="" class="h-40 w-40">

                            <input id="ipt" type="file" placeholder="image" name="image" value="" onchange="showPreview()"
                ...
                <div class="mt-4">
                    <label class="block" for="email">Caption<label>
                            <input type="text" placeholder="caption" name="caption" value="{{$post -> caption}}"
                                class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
								@error('caption')
							<div class="text-xs text-red-400">
								{{$message}}
                ...
                <div class="flex">
                    <button class="w-full px-6 py-2 mt-4 text-white bg-blue-600 rounded-lg hover:bg-blue-900">Confirm and Upload</button>
                </div>
